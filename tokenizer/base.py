...
def tokenf(pattern, strings):
    
    pattern = regex.compile(pattern) if type(pattern) == str else pattern
        
    return regex.findall(pattern, strings)
...
class BaseTokenizer:
    ...
    def encode(self, corpora):
        ...
        chunks = tokenf(self.string, corpora); encode = list()
